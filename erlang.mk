...
$(ERL) -pa $(CURDIR)/ebin $(DEPS_DIR)/*/ebin \
    -eval "try triq:module_info() of _ -> ok catch error:undef -> halt() end." \
    $(TRIQ_RUN_BEFORE) \
	-eval "try $(1) of true -> ok; _ -> halt(1) \
	    catch error:undef -> io:format(\"Undefined property or module~n\"), halt(1) end." \
    $(TRIQ_RUN_AFTER) \
    -eval "halt(0)."
endef
...
triq: test-build
	$(eval MODULES := $(shell find ebin -type f -name \*.beam \
		| sed "s/ebin\//'/;s/\.beam/',/" | sed '$$s/.$$//'))
	$(gen_verbose) $(call triq_run,lists:all(fun triq:check/1, [$(MODULES)]))
endif
...
# Hook in coverage to triq.

ifdef COVER
ifdef triq_run
TRIQ_RUN_BEFORE += -eval \
	'case cover:compile_beam_directory("ebin") of \
		{error, _} -> halt(1); \
		_ -> ok \
	end.'
TRIQ_RUN_AFTER += -eval 'cover:export("triq.coverdata").'
endif
endif
